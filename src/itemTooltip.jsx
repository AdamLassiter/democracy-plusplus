...
function PropertySection({ title, value, depth = 0 }) {
  ...
  const primitiveRows = entries.filter(([, entryValue]) => isPrimitive(entryValue));
  const nestedRows = entries.filter(([, entryValue]) => !isPrimitive(entryValue));
