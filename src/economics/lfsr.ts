...
export class LFSR {
    ...
    shift() {
        var tapsNum = this.taps.length, i: number, bit = this.register >> (this.n - this.taps[0]);
        for (i = 1; i < tapsNum; i++) {
     ...
     */
    maxSeqLen() {
        var initialState = this.register, counter = 0;
        do {
            this.shift();
            counter++;
...
export class PRNG {
    ...
    rand(min: number, max: number) {
        ...
        var bits = ~~this._log2(max - offset) + 1, random;
        do {
