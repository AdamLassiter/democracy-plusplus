...
import { playerCountMultiplier } from "../utils/playerCount";
...
export function calculateMissionReward(mission: MissionWithStars) {
  ...
  const players = playerCountMultiplier(mission.playerCount);

  return Math.round(BASE_REWARD * stars * missionModifier * players);
}
